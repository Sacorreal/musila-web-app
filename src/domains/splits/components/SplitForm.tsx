...
export function SplitForm({ trackId, existingSplit, onDone }: Props) {
  ...
  const { control, handleSubmit, watch, setValue, formState: { errors, isValid } } =
    useForm<CreateSplitFormValues>({
      ...
      mode: "onChange",
      defaultValues: {
        // Solo coautores persona: la publisher se reinyecta server-side al guardar.
        authors: existingSplit
          ? existingSplit.authors
              .filter((a) => a.user)
              .map((a) => ({
                userId: a.user!.id,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
      ...
      <Button type="submit" disabled={!isValid || !sumIsValid || isSaving || fields.length === 0} className="w-full gap-2">
