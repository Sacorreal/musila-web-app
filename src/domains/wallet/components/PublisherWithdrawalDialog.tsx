...
export function PublisherWithdrawalDialog({ organizationId }: { organizationId: string }) {
  const [open, setOpen] = useState(false);
  const { data: balance } = usePublisherWalletBalance(organizationId);
  const { data: bankAccount } = usePublisherBankAccount(organizationId);
  const { mutate: createWithdrawal, isPending } = useCreatePublisherWithdrawal(organizationId);
      ...
      <DialogContent>
        ...
        {!hasBankAccount ? (
          <p className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-sm text-muted-foreground">
            Completa los datos bancarios de la organización antes de solicitar un retiro.
          </p>
        ) : (
