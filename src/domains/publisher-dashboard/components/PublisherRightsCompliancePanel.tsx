...
import { usePublisherRightsCompliance } from '../hooks/publisher-dashboard.hooks'

export function PublisherRightsCompliancePanel({ organizationId }: { organizationId: string }) {
  const { data, isLoading, isError } = usePublisherRightsCompliance(organizationId)
  ...
  if (isError || !data) {
    return <ErrorState message="No se pudo cargar el cumplimiento de derechos del catálogo." />
  }
