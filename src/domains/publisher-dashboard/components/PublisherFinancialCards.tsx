...
} from '@/src/domains/dashboard/components/DashboardCardSkeleton'
import { usePublisherFinancial } from '../hooks/publisher-dashboard.hooks'
...
export function PublisherFinancialCards({ organizationId }: { organizationId: string }) {
  const { data, isLoading, isError } = usePublisherFinancial(organizationId)
  ...
  if (isError || !data) {
    return <ErrorState message="No se pudo cargar el resumen financiero de la organización." />
  }
      ...
      <Card>
        ...
        <CardContent>
          {data.nextPayment ? (
            ...
            <p className="text-sm text-muted-foreground">
              No hay cuotas de licencia pendientes por cobrar en el catálogo.
            </p>
          )}
        </CardContent>
      </Card>
