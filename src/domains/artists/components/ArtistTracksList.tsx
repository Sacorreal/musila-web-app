    ...
    <div className="mt-10 px-2 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <Button className="w-14 h-14 rounded-full bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 p-0 flex items-center justify-center align-middle shadow-lg border-0 shrink-0">
          <Play className="w-6 h-6 text-foreground dark:text-slate-300 ml-1" fill="currentColor" />
        </Button>
      ...
      <div className="flex flex-col gap-2">
        {populatedTracks.length === 0 ? (
          ...
          filteredTracks.map((track, index) => (
            ...
            >
              <span className="text-muted-foreground font-medium w-4 shrink-0 text-center">{index + 1}</span>

              <div className="flex items-center gap-4 flex-1 min-w-0">
                {track.coverUrl ? (
                  <img src={track.coverUrl} alt={track.title} className="w-12 h-12 rounded object-cover shadow-sm bg-slate-200 dark:bg-slate-800" />
                ) : (
                ...
                <span className="text-foreground font-semibold truncate hover:underline">{track.title}</span>
              </div>
