...
import { Play, MoreHorizontal, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export default function GenreDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const filteredTracks = useMemo(() => {
    ...
    return genre.tracks.filter(track => {
      ...
      if (subGenreFilter !== "all" && track.subGenre !== subGenreFilter) return false;
  ...
  if (isError || !genre) {
    return (
      <div className="container mx-auto p-8 text-center">
        <h1 className="text-2xl font-bold text-foreground">Género no encontrado</h1>
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 space-y-12 text-foreground">
      ...
      <div className="space-y-6 pt-8">
        <h1 className="text-6xl font-black text-foreground tracking-tighter uppercase">{genre.genre}</h1>
        ...
        <div className="flex flex-wrap items-center gap-4 py-2">
          <div className="flex items-center gap-3 bg-slate-100 dark:bg-slate-900/80 px-4 py-2 rounded-xl border border-slate-300 dark:border-white/5 shadow-inner">
            <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Gospel</span>
            <Switch checked={isGospelFilter} onCheckedChange={setIsGospelFilter} />
          </div>
      ...
      <div className="bg-card dark:bg-slate-900/20 rounded-[2.5rem] overflow-hidden border border-slate-300 dark:border-white/5 backdrop-blur-sm shadow-xl">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-white/5">
              {filteredTracks.map((track, index) => (
                <tr key={track.id} className="group hover:bg-slate-50 dark:hover:bg-white/[0.03] transition-all duration-300 cursor-pointer">
                ...
                <tr>
                  <td colSpan={7} className="px-8 py-24 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-muted-foreground font-bold uppercase tracking-widest text-sm">No hay pistas disponibles</p>
                    </div>
                  </td>
                </tr>
              )}
        ...
        {filteredTracks.length > 0 && (
          <div className="p-8 flex items-center justify-center gap-3 border-t border-white/5">
            <Button variant="ghost" size="icon" className="w-10 h-10 rounded-full text-slate-500 hover:text-white hover:bg-white/10">
              <ChevronLeft size={20} />
            </Button>
            <Button className="w-10 h-10 rounded-full bg-blue-500 text-white font-black text-xs p-0 shadow-lg shadow-blue-500/20">1</Button>
            <Button variant="ghost" className="w-10 h-10 rounded-full text-slate-500 font-black text-xs p-0 hover:text-white hover:bg-white/10">2</Button>
            <Button variant="ghost" className="w-10 h-10 rounded-full text-slate-500 font-black text-xs p-0 hover:text-white hover:bg-white/10">3</Button>
            <Button variant="ghost" size="icon" className="w-10 h-10 rounded-full text-slate-500 hover:text-white hover:bg-white/10">
      ...
      <section className="space-y-10 pb-20">
        <div className="flex items-center gap-4">
          <h2 className="text-3xl font-black text-foreground tracking-tighter uppercase">Autores Destacados</h2>
          <div className="h-[1px] flex-1 bg-gradient-to-r from-border to-transparent" />
        </div>
        
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-10">
          {(uniqueAuthors.length > 0 ? uniqueAuthors.slice(0, 6) : [
            { name: "Nathaniel Hayes", img: "https://i.pravatar.cc/150?u=1" },
            { name: "Jonathan Reed", img: "https://i.pravatar.cc/150?u=2" },
            { name: "Isabella Foster", img: "https://i.pravatar.cc/150?u=3" },
            { name: "Charles Bennett", img: "https://i.pravatar.cc/150?u=4" },
